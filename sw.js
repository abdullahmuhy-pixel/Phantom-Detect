...
// Cache version is passed as ?v= from the HTML registration.
// It's set to the page's last-modified timestamp, so it auto-busts
// whenever index.html is updated — no manual versioning needed.
const CACHE_VERSION = new URL(self.location.href).searchParams.get('v') || 'default';
const CACHE_NAME = `phantom-detect-${CACHE_VERSION}`;

// Build paths relative to SW scope so this works on any subdirectory/domain
...
self.addEventListener('fetch', event => {
  ...
  // App shell — cache-first for app shell, network-first for API
  event.respondWith(
